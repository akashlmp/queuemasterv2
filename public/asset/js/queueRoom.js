...
var MultistepForm = {
  ...
  nextStep: function () {
    if (this.currentStep < this.totalSteps - 1) {
      if (this.isValidStep()) { // Check if the current step is valid
        this.currentStep++;
        this.updateSteps();
      } else {
        alert("Please fill in all required fields highlighted with (*).");
      }
    }
  },
  ...
  isValidStep: function() {
    const currentPane = document.querySelectorAll('.tab-pane')[this.currentStep];
    const inputs = currentPane.querySelectorAll('input[required], textarea[required], select[required], radio[required]');
...
// Optional: add a class to visually indicate error
document.addEventListener("DOMContentLoaded", function() {
  const css = `.error { border: 1px solid red; }`,
...
$(document).ready(function () {
  $("#AdvanceSettingCheckBox").change(function () {
    if ($(this).is(":checked")) {
      $(".AdvanceSettingBox").slideDown();
    } else {
      $(".AdvanceSettingBox").slideUp();
    }
  });
});

const addButton = document.getElementById("addButton");
const table = document.querySelector("#AdvanceSettingTable tbody");
...
function showPlusButton() {
  ...
  const lastRowPlusButtons =
    table.lastElementChild.querySelector(".AddTableRow");
...
function showDatePicker() {
  ...
  datePickerIds.forEach((datePickerId) => {
	...
	if(typeof custom_end_date == 'undefined' && 'custom_end_date' == datePickerId) {
		picker.setDate(new Date());
	} else if(typeof custom_start_date == 'undefined' && 'custom_start_date' == datePickerId) {
		picker.setDate(new Date());
	} else {
  ...
  timePickerIds.forEach((timePickerId) => {
	...
	var allowtim = 0;
	if(typeof custom_end_date == 'undefined' && 'EndtimePicker' == timePickerId) {
		allowtim = 1;
	} else if(typeof custom_start_date == 'undefined' && 'timePicker' == timePickerId) {
		allowtim = 1;
	}
	if(allowtim == 1) {
...
function addDynamicSelect(
  ...
  deleteButton.addEventListener("click", function () {
    ...
    if (index !== -1) {
      ...
      document.getElementById("jsonlangDesignTemp").value =
        JSON.stringify(languageDesignTemp);
    }
...
document
  .getElementById("mainSelectDesignTemp")
